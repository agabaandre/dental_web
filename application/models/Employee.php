...
class Employee extends CI_Model
{
    ...
    public function get_bill(){
        ...
        $query=$this->db->query("SELECT sum(amount) as totalbill,description, posting_date,bill.appointment_id,name,mobile,bill_status,partial_payment,posted_by FROM bill left join appointments on appointments.id=bill.appointment_id left join request on request.id=appointments.request_id group by bill.appointment_id
        ");
        if ($query){
            return $query->result();
            }
    ...
    public function print_bill($id){
        ...
        $query=$this->db->query("SELECT sum(amount) as totalbill,description, posting_date,bill.appointment_id,name,mobile,bill_status,partial_payment,posted_by FROM bill left join appointments on appointments.id=bill.appointment_id left join request on request.id=appointments.request_id where bill.appointment_id='$id' group by appointment_id
        ");
        if ($query){
            return $query->result();
            }
